...
function createAdminNotifications(requestRecord) {
  return getSuperAdminEmails().map((recipientEmail) => ({
    ...
    trash: false,
    resolved_at: null,
    title: requestRecord.type === "request-access" ? "Solicitação de acesso" : "Primeiro acesso",
...
async function resolveRequest(supabase, {
  ...
  await writeAuditLog({
    ...
    entityId: requestId,
    route: "lib/access-requests-store",
    details: {
      requestEmail: requestRecord.email,
      requestType: requestRecord.type,
      requestedRole: requestRecord.requested_role,
      resolvedByName: actorName || "",
    },
  }).catch(() => null);

  await writeSystemEvent({
    event: `access_request.${decision}`,
    level: "info",
    route: "lib/access-requests-store",
    actorUserId,
    actorEmail,
    message: `Solicitacao ${decision} para ${requestRecord.email}.`,
    meta: {
      requestId,
      requestType: requestRecord.type,
      requestedRole: requestRecord.requested_role,
...
export async function queueAccessRequest({ email, type }) {
  ...
  await writeSystemEvent({
    ...
    message: `Nova solicitacao de acesso criada para ${normalizedEmail}.`,
    meta: {
...
export async function rejectAccessRequest({ requestId, actorUserId, actorEmail, actorName }) {
  assertAdminEnv();
  const supabase = createAdminClient();
...
export async function approveAccessRequest({
  ...
  try {
    ...
    await writeSystemEvent({
      event: "access_request.approval_failed",
      level: "error",
      route: "lib/access-requests-store",
      actorUserId,
      actorEmail,
