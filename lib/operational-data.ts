...
function getOperationalDataPlan(scope = "default") {
  const normalizedScope = normalizeText(scope, "default");

  if (normalizedScope === "sellers") {
    return {
      meetings: true,
      logs: false,
      notifications: false,
...
export async function enrichDashboardWithOperationalData(dashboardData, user, options = {}) {
  const plan = getOperationalDataPlan(options.scope);
  const [persistedMeetings, logData, notifications] = await Promise.all([
    plan.meetings ? listPersistedMeetingsForUser(user).catch(() => []) : Promise.resolve([]),
    plan.logs ? listOperationalLogsForUser(user).catch(() => ({ auditLogs: [], syncLogs: [] })) : Promise.resolve({ auditLogs: [], syncLogs: [] }),
    plan.notifications ? listOperationalNotificationsForUser(user) : Promise.resolve([]),
  ]);
  ...
  return {
    ...dashboardData,
    meetings: plan.meetings ? dedupeMeetings([...persistedMeetings, ...hubspotMeetings]).sort((left, right) => {
