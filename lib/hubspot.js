...
function getHubSpotCacheKey(scope) {
  return `${getAppEnvironment()}:${getHubSpotTokenSource()}:${scope}`;
}

function readHubSpotDashboardCache(scope) {
  const entry = hubSpotDashboardCache.get(getHubSpotCacheKey(scope));
  if (!entry) {
    return null;
  }
  ...
  if (!entry.promise) {
    hubSpotDashboardCache.delete(getHubSpotCacheKey(scope));
  }
