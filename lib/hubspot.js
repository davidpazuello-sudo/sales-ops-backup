...
function getHubSpotCacheKey(scope, options = {}) {
  ...
  return normalizedPipelineId
    ? `${getAppEnvironment()}:${getHubSpotTokenSource()}:${scope}:${normalizedPipelineId}`
    : `${getAppEnvironment()}:${getHubSpotTokenSource()}:${scope}`;
}
...
const HUBSPOT_SCOPE_PLANS = {
  default: {
    owners: { maxPages: 2, maxRecords: 100 },
  ...
  campaigns: {
    owners: { maxPages: 2, maxRecords: 100 },
    deals: { includeCampaigns: true, includeContacts: true, maxPages: 2, maxRecords: 120 },
    contacts: { includeCampaigns: true, includeDeals: true, maxPages: 2, maxRecords: 120 },
    tasks: { includeCampaigns: true, includeAssociations: true, maxPages: 1, maxRecords: 60 },
    calls: { includeCampaigns: true, includeAssociations: true, maxPages: 1, maxRecords: 60 },
    meetings: { includeCampaigns: true, includeAssociations: true, maxPages: 1, maxRecords: 60 },
  },
...
function buildDealSearchBody(pipelineId, options = {}) {
  const properties = [
    ...HUBSPOT_DEAL_BASE_PROPERTIES,
    ...(options.includeCampaigns ? HUBSPOT_CAMPAIGN_PROPERTIES : []),
  ];
...
export async function getHubSpotDashboardData(options = {}) {
  ...
  const cachedData = readHubSpotDashboardCache(scope, { pipelineId: requestedPipelineId });
  if (cachedData) {
    return cachedData;
  }

  const cacheKey = getHubSpotCacheKey(scope, { pipelineId: requestedPipelineId });
  ...
  const loadPromise = (async () => {
    ...
    if (plan.owners) {
      requests.push(fetchPagedCollection(HUBSPOT_OWNER_FIELDS[0], plan.owners));
    } else {
    ...
    if (plan.contacts) {
      requests.push(fetchOptionalPagedCollection(buildContactsPath(plan.contacts), plan.contacts));
    } else {
      requests.push(Promise.resolve([]));
    }
    ...
    const deals = scope === "deals" && plan.deals
      ? (selectedPipelineId
        ? await fetchSearchedCollection(
          ...
          buildDealSearchBody(selectedPipelineId, plan.deals),
          plan.deals,
        )
        : [])
      : initialDeals;
