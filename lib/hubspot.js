...
const HUBSPOT_SELLERS_FIRST_PAGE_SIZE = 10;
...
function paginateSellerOwners(owners = [], sellerPage = "1") {
  return normalizeSellerPage(sellerPage) === "2"
    ? owners.slice(HUBSPOT_SELLERS_FIRST_PAGE_SIZE)
    : owners.slice(0, HUBSPOT_SELLERS_FIRST_PAGE_SIZE);
}
...
export async function getHubSpotDashboardData(options = {}) {
  ...
  const loadPromise = (async () => {
    ...
    const selectedOwners = sellersScope ? paginateSellerOwners(sortedOwners, requestedSellerPage) : owners;
    ...
    if (sellersScope) {
      ...
      dashboardData.sellerPagination = {
        currentPage: Number(requestedSellerPage),
        totalPages: sortedOwners.length > HUBSPOT_SELLERS_FIRST_PAGE_SIZE ? 2 : 1,
        totalOwners: sortedOwners.length,
