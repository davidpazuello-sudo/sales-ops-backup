...
function resolveSelectedOwnerId(owners = [], requestedOwnerFilter = "") {
  const normalizedRequestedOwner = normalizeComparable(requestedOwnerFilter);
  if (!normalizedRequestedOwner || normalizedRequestedOwner === "todos") {
    return "";
  }
  ...
  return String(
    owners.find((owner) => {
      ...
      return ownerName === normalizedRequestedOwner || ownerEmail === normalizedRequestedOwner;
    })?.id || "",
  ).trim();
}
...
function filterHubSpotRecordsByOwner(records = [], ownerId = "") {
  const normalizedOwnerId = String(ownerId || "").trim();
  if (!normalizedOwnerId) {
    return records;
  }

  return records.filter((record) => String(record?.properties?.hubspot_owner_id || "").trim() === normalizedOwnerId);
}

function getAssociationIdsFromRecord(record, associationType) {
  return (record?.associations?.[associationType]?.results || [])
    .map((item) => String(item?.id || "").trim())
    .filter(Boolean);
}

function matchesSelectedCampaignLabel(value, requestedCampaignName = "") {
  const normalizedValue = normalizeComparable(value);
  const normalizedRequestedCampaignName = normalizeComparable(requestedCampaignName);

  if (!normalizedValue || !normalizedRequestedCampaignName) {
    return false;
  }
  ...
  if (isPrimaryCampaignName(requestedCampaignName) && isPrimaryCampaignName(value)) {
    return true;
  }
...
function matchesSelectedCampaignTokens(value, requestedCampaignName = "") {
  const normalizedValue = normalizeComparable(value);
  const requestedTokens = normalizeComparable(requestedCampaignName)
    .split(/\s+/)
    .filter(Boolean);

  if (!normalizedValue || !requestedTokens.length) {
    return false;
  }

  return requestedTokens.every((token) => normalizedValue.includes(token));
}

function filterCampaignActivitiesByAssociations(records = [], { contacts = [], deals = [], requestedCampaignName = "" } = {}) {
  ...
  const campaignDealIds = new Set(
    deals
      .map((deal) => String(deal?.id || "").trim())
      .filter(Boolean),
  );

  return dedupeHubSpotRecords(records.filter((record) => {
    const properties = record?.properties || {};
    const directCampaignLabel = extractCampaignLabel(properties);

    if (matchesSelectedCampaignLabel(directCampaignLabel, requestedCampaignName)) {
      return true;
    }
    ...
    if (matchesSelectedCampaignTokens(titleOrBody, requestedCampaignName)) {
      return true;
    }

    const associatedContactIds = getAssociationIdsFromRecord(record, "contacts");
    if (associatedContactIds.some((contactId) => campaignContactIds.has(contactId))) {
      return true;
    }

    const associatedDealIds = getAssociationIdsFromRecord(record, "deals");
    if (associatedDealIds.some((dealId) => campaignDealIds.has(dealId))) {
      return true;
    }
...
function getHubSpotCacheKey(scope, options = {}) {
  ...
  if ((scope === "campaigns" || scope === "presales") && normalizedCampaignName) {
...
async function fetchListByName(listName = "") {
...
function resolveHubSpotListId(payload) {
  return String(
    payload?.listId
      || payload?.id
      || payload?.list?.listId
...
async function fetchListMembershipIds(listId = "", options = {}) {
  ...
  return [...new Set(
    results
      .map((item) => String(item?.recordId || item?.id || item?.entityId || "").trim())
      .filter(Boolean),
  )];
}

async function fetchContactsByIds(contactIds = [], options = {}) {
  ...
  const results = await Promise.all(
    chunks.map((chunk) => hubspotFetch("/crm/v3/objects/contacts/batch/read", {
      method: "POST",
    ...
    }).then((payload) => payload?.results || []).catch(() => [])),
  );

  return dedupeHubSpotRecords(results.flat());
}

function mergeHubSpotContactRecords(baseContacts = [], historyContacts = []) {
...
async function fetchContactsFromCampaignSegmentLists(campaignName = "", options = {}) {
  const configuredListNames = getConfiguredCampaignSegmentListNames(campaignName);
  if (!configuredListNames.length) {
    return [];
  }

  const lists = await Promise.all(configuredListNames.map((listName) => fetchListByName(listName)));
  const membershipIds = await Promise.all(
    lists
      .filter(Boolean)
      .map((list) => fetchListMembershipIds(resolveHubSpotListId(list), options).catch(() => [])),
  );
...
const HUBSPOT_SCOPE_PLANS = {
  ...
  presales: {
    owners: { maxPages: 2, maxRecords: 500 },
    deals: { includeCampaigns: false, includeContacts: true, maxPages: 4, maxRecords: 400 },
    contacts: { includeCampaigns: false, includeDeals: true, maxPages: 8, maxRecords: 800 },
    tasks: { includeCampaigns: false, includeAssociations: true, maxPages: 5, maxRecords: 420 },
    calls: { includeCampaigns: false, includeAssociations: true, maxPages: 5, maxRecords: 420 },
    meetings: { includeCampaigns: false, includeAssociations: true, maxPages: 5, maxRecords: 420 },
  },
  deals: {
    owners: { maxPages: 2, maxRecords: 500 },
  ...
  campaigns: {
    ...
    tasks: { includeCampaigns: true, includeAssociations: true, maxPages: 8, maxRecords: 800 },
    calls: { includeCampaigns: true, includeAssociations: true, maxPages: 8, maxRecords: 800 },
    meetings: { includeCampaigns: true, includeAssociations: true, maxPages: 8, maxRecords: 800 },
  },
  tasks: {
    owners: { maxPages: 2, maxRecords: 500 },
...
function mergeConfiguredCampaignOptions(options = []) {
  const merged = new Map(
    options.map((option) => [normalizeComparable(option?.label), option]),
  );
...
export async function getHubSpotDashboardData(options = {}) {
  ...
  const loadPromise = (async () => {
        ...
        filteredCampaignScope
          ...
          : (scope === "tasks"
            ? fetchSearchedCollection(
              ...
              buildUpcomingActivitySearchBody("tasks", HUBSPOT_TASK_BASE_PROPERTIES, plan.tasks),
              plan.tasks,
            ).catch(() => [])
        ...
        filteredCampaignScope
          ...
          : (scope === "tasks"
            ? fetchSearchedCollection(
              ...
              buildUpcomingActivitySearchBody("calls", HUBSPOT_CALL_BASE_PROPERTIES, plan.calls),
              plan.calls,
            ).catch(() => [])
        ...
        filteredCampaignScope
          ...
          : (scope === "tasks"
            ? fetchSearchedCollection(
              ...
              buildUpcomingActivitySearchBody("meetings", HUBSPOT_MEETING_BASE_PROPERTIES, plan.meetings),
              plan.meetings,
            ).catch(() => [])
    ...
    const [owners, pipelines, initialDeals, initialContacts, tasks, calls, meetings] = await Promise.all(
    ...
    const listBackedContacts = (filteredCampaignScope || presalesCampaignScope)
      ? await fetchContactsFromCampaignSegmentLists(requestedCampaignName, plan.contacts).catch(() => [])
      : [];
    const baseCampaignContacts = (filteredCampaignScope || presalesCampaignScope)
      ? (listBackedContacts.length ? listBackedContacts : initialContacts)
      : initialContacts;
    const historyBackedContacts = (campaignScope || presalesScope) && requestedCampaignName && baseCampaignContacts.length
      ? await fetchContactsByIds(baseCampaignContacts.map((contact) => contact?.id), plan.contacts).catch(() => [])
      : [];
    ...
    const selectedPipelineId = scope === "deals"
      ...
      : "";
    const selectedOwnerId = (scope === "deals" || scope === "campaigns" || scope === "presales") && requestedOwnerFilter && requestedOwnerFilter !== "todos"
      ? resolveSelectedOwnerId(sortedOwners, requestedOwnerFilter)
      : "";
    const presalesOwnerIds = presalesScope && selectedOwnerId ? [selectedOwnerId] : [];
    const presalesContacts = presalesOwnerIds.length && plan.contacts
    ...
    const sellerMeetings = sellersScope
      ...
      : (presalesOwnerIds.length
        ...
        : meetings);
    const contacts = campaignScope || presalesCampaignScope
      ? mergeHubSpotContactRecords(baseCampaignContacts, historyBackedContacts)
      : presalesContacts;
    ...
    const campaignScopedTasks = filteredCampaignScope
      ? filterCampaignActivitiesByAssociations(ownerFilteredTasks, {
        contacts: ownerFilteredContacts,
        deals: ownerFilteredDeals,
        requestedCampaignName,
      })
      : sellerTasks;
    const campaignScopedCalls = filteredCampaignScope
      ? filterCampaignActivitiesByAssociations(ownerFilteredCalls, {
        contacts: ownerFilteredContacts,
        deals: ownerFilteredDeals,
        requestedCampaignName,
      })
      : sellerCalls;
    const campaignScopedMeetings = filteredCampaignScope
      ? filterCampaignActivitiesByAssociations(ownerFilteredMeetings, {
        contacts: ownerFilteredContacts,
        deals: ownerFilteredDeals,
        requestedCampaignName,
      })
    ...
    const campaignOptions = scope === "campaigns" || scope === "presales"
      ? mergeConfiguredCampaignOptions(extractCampaignOptionsFromRecords(initialDeals, contacts))
      : [];
