...
import { buildPreSalesSummary } from "./services/dashboard-presales";
...
const HUBSPOT_CONTACT_QUALIFIED_STATUS_LABELS = {
  Sim: "Sim",
  "Não": "Não",
  Nao: "Não",
};
...
function normalizeHubSpotQualifiedStatus(value) {
  const normalizedValue = normalizeString(value);
  return HUBSPOT_CONTACT_QUALIFIED_STATUS_LABELS[normalizedValue] || normalizedValue;
}
...
function normalizeHubSpotHistorySourceIds(sourceId) {
  ...
  const hubSpotUserIdMatch = normalizedSource.match(/^\{\s*userId\s*:\s*"?([^"}]+)"?\s*\}$/i);
  if (hubSpotUserIdMatch) {
    return normalizeHubSpotHistorySourceIds({ userId: hubSpotUserIdMatch[1] });
  }

  const hubSpotPrefixedUserIdMatch = normalizedSource.match(/^userId:([^\s]+)$/i);
  if (hubSpotPrefixedUserIdMatch) {
    return normalizeHubSpotHistorySourceIds({ userId: hubSpotPrefixedUserIdMatch[1] });
  }
...
function resolveQualifiedStatusUpdatedBy(contact, owners = []) {
  const qualifiedHistory = contact?.propertiesWithHistory?.conectado;
  if (!Array.isArray(qualifiedHistory) || !qualifiedHistory.length) {
    return "";
  }

  const latestNotQualifiedEntry = [...qualifiedHistory]
    .filter((entry) => normalizeComparable(normalizeHubSpotQualifiedStatus(entry?.value)) === "nao")
    .sort((left, right) => new Date(right?.timestamp || 0).getTime() - new Date(left?.timestamp || 0).getTime())
    [0];

  if (!latestNotQualifiedEntry) {
    return "";
  }

  return resolveHubSpotHistoryActor(latestNotQualifiedEntry, owners);
}

function resolveQualifiedStatusNoActors(contact, owners = []) {
  const qualifiedHistory = contact?.propertiesWithHistory?.conectado;
  if (!Array.isArray(qualifiedHistory) || !qualifiedHistory.length) {
    return [];
  }
  ...
  return [...new Set(
    qualifiedHistory
      .filter((entry) => normalizeComparable(normalizeHubSpotQualifiedStatus(entry?.value)) === "nao")
      .map((entry) => resolveHubSpotHistoryActor(entry, owners))
      .map((actor) => normalizeString(actor))
      .filter(Boolean),
  )];
}
...
export function normalizeHubSpotContact(contact, ownerMap, owners = []) {
  ...
  return {
    ...
    qualifiedStatus: normalizeHubSpotQualifiedStatus(properties.conectado),
    ...
    qualifiedUpdatedBy: normalizeString(resolveQualifiedStatusUpdatedBy(contact, owners)),
    qualifiedNoActors: resolveQualifiedStatusNoActors(contact, owners),
...
export function buildDashboardDomainPayload(rawOwners, rawDeals, rawActivities = {}, rawPipelines = [], options = {}) {
  ...
  const presales = buildPreSalesSummary({
    contacts,
    deals,
    activities: activityItems,
    ownerFilter: options.selectedOwnerFilter || "todos",
    campaignName: options.selectedCampaignName || "",
  });
