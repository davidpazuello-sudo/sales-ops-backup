...
export function getHubSpotClientSecret() {
  ...
  if (environment === "staging") {
    return process.env.HUBSPOT_CLIENT_SECRET_STAGING
      || process.env.HUBSPOT_CLIENT_SECRET
      || "";
  }

  if (environment === "production") {
    return process.env.HUBSPOT_CLIENT_SECRET_PRODUCTION
      || process.env.HUBSPOT_CLIENT_SECRET
      || "";
  }

  return process.env.HUBSPOT_CLIENT_SECRET_DEVELOPMENT
    || process.env.HUBSPOT_CLIENT_SECRET
    || "";
}

export function hasHubSpotClientSecretConfigured() {
  return Boolean(getHubSpotClientSecret());
}

export function getHubSpotClientSecretSource() {
  const environment = getAppEnvironment();

  if (environment === "staging" && process.env.HUBSPOT_CLIENT_SECRET_STAGING) {
    return "HUBSPOT_CLIENT_SECRET_STAGING";
  }

  if (environment === "production" && process.env.HUBSPOT_CLIENT_SECRET_PRODUCTION) {
    return "HUBSPOT_CLIENT_SECRET_PRODUCTION";
  }

  if (environment === "development" && process.env.HUBSPOT_CLIENT_SECRET_DEVELOPMENT) {
    return "HUBSPOT_CLIENT_SECRET_DEVELOPMENT";
  }
