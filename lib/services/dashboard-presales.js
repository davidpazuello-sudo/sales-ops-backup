...
function matchesCampaign(value, selectedCampaignName) {
  const normalizedCampaign = normalizeComparable(selectedCampaignName);
  if (!normalizedCampaign) {
    return true;
  }

  return normalizeComparable(value) === normalizedCampaign;
}
...
export function buildPreSalesSummary({
  ...
  const filteredContacts = contacts.filter((contact) => (
    (matchesOwner(contact.ownerName, ownerFilter) || matchesOwner(contact.ownerEmail, ownerFilter))
    && matchesCampaign(contact.campaignName, campaignName)
  ));
  ...
  const ownerDeals = deals
    .filter((deal) => intersectsIds(deal.contactIds, filteredContactIds) && matchesCampaign(deal.campaignName, campaignName))
    .sort(sortByDateDesc);
