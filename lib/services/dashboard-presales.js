...
function matchesOwner(value, ownerFilter) {
  if (!ownerFilter || normalizeComparable(ownerFilter) === "todos") {
    return true;
  }

  return normalizeComparable(value) === normalizeComparable(ownerFilter);
}
...
export function buildPreSalesSummary({
  ...
  const filteredContacts = contacts.filter((contact) => matchesOwner(contact.ownerName, ownerFilter));
  ...
  const contactsWithoutConnection = filteredContacts
    ...
    .sort(sortByDateDesc);
  const ownerDeals = deals.filter((deal) => intersectsIds(deal.contactIds, filteredContactIds));
