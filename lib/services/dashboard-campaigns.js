...
export function extractCampaignLabel(properties = {}) {
  const explicitPrimaryMarker = inferPrimaryCampaignLabel(
    properties.alunos_a_bordo_contatos,
    properties.dealname,
  );

  if (explicitPrimaryMarker) {
    return explicitPrimaryMarker;
  }
...
export function buildCampaignSummaries({
  ...
  for (const campaign of campaignMap.values()) {
    ...
    const totalLeadItems = campaignContacts.map((contact) => ({
      id: contact.id,
    ...
    const sqlLeadItems = campaignContacts
      .filter(isSql)
      .map((contact) => ({
        id: contact.id,
        ownerName: contact.ownerName || "Sem responsavel",
    ...
    const closedWonItems = campaignDeals
    ...
    const qualifiedOpportunityItems = campaignDeals
    ...
    const meetingItems = campaignActivities
