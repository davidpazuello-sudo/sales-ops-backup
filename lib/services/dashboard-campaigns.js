...
function isOwnedByCecilia(contact = {}) {
  const lifecycleUpdatedBy = normalizeComparable(contact.lifecycleUpdatedBy);
  return lifecycleUpdatedBy.includes("cecilia") || lifecycleUpdatedBy.includes("mercado privado");
}
...
export function buildCampaignSummaries({
  ...
  for (const campaign of campaignMap.values()) {
    ...
    const ceciliaDisqualifiedContacts = disqualifiedContacts.filter(isOwnedByCecilia);
    ...
    const ceciliaDisqualifiedNumberItems = shouldIncludeCampaignDetail(includeDetails, "ceciliaDisqualifiedNumbers", detailKey)
      ? ceciliaDisqualifiedContacts.map((contact) => buildAttemptDetailItem(contact, contact.leadStatus || "Desqualificado"))
      : [];
