...
export function isPrimaryCampaignName(value) {
  return normalizeComparable(value) === normalizeComparable(PRIMARY_CAMPAIGN_NAME);
}
...
export function buildCampaignSummaries({
  ...
  for (const campaign of campaignMap.values()) {
    const campaignDeals = filteredDeals.filter((deal) => slugify(deal.campaignName || "Sem campanha identificada") === campaign.id);
    const campaignContacts = filteredContacts.filter((contact) => slugify(contact.campaignName || "Sem campanha identificada") === campaign.id);
    const campaignActivities = filteredActivities.filter((activity) => slugify(activity.campaignName || "Sem campanha identificada") === campaign.id);
