...
function buildProspectingAttemptMap(contacts = [], activities = []) {
  const prospectingActivities = activities.filter((activity) => activity.kind !== "meeting");
  const attemptsByContactId = new Map();

  prospectingActivities.forEach((activity) => {
    const associatedContactIds = Array.isArray(activity.contactIds) ? activity.contactIds.filter(Boolean) : [];
    associatedContactIds.forEach((contactId) => {
      attemptsByContactId.set(contactId, (attemptsByContactId.get(contactId) || 0) + 1);
    });
  });

  return contacts.map((contact) => ({
    ...contact,
    attemptCount: attemptsByContactId.get(contact.id) || 0,
  }));
}
...
export function buildCampaignSummaries({
  ...
  for (const campaign of campaignMap.values()) {
    ...
    const contactAttempts = buildProspectingAttemptMap(campaignContacts, campaignActivities);
    ...
    const firstAttemptContacts = contactAttempts.filter((contact) => contact.attemptCount === 1);
    const secondAttemptContacts = contactAttempts.filter((contact) => contact.attemptCount === 2);
    const thirdAttemptContacts = contactAttempts.filter((contact) => contact.attemptCount === 3);
    const fourthAttemptContacts = contactAttempts.filter((contact) => contact.attemptCount >= 4);
