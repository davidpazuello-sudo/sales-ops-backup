...
export const PRIMARY_CAMPAIGN_SEGMENT_LIST_NAMES = [
  "Contatos Norte - Aluno a Bordo 2026",
  "Secretarias Norte - Aluno a Bordo 2026",
];
...
function collectActivityContactIds(activity = {}, dealMap = new Map()) {
  const directContactIds = Array.isArray(activity.contactIds) ? activity.contactIds : [];
  const viaDealContactIds = (Array.isArray(activity.dealIds) ? activity.dealIds : [])
    .flatMap((dealId) => dealMap.get(dealId)?.contactIds || []);

  return [...directContactIds, ...viaDealContactIds].filter(Boolean);
}

function countUniqueContactsWithActivity(activities, contacts = [], dealMap = new Map(), kind, mode = "day") {
  const currentBucket = formatDateKey(new Date().toISOString(), mode);
  if (!currentBucket) {
    return 0;
  }

  const validContactIds = new Set(contacts.map((contact) => contact?.id).filter(Boolean));
  const matchedContactIds = new Set();

  activities
    .filter((activity) => !kind || activity.kind === kind)
    .filter((activity) => formatDateKey(activity.dueAt || activity.updatedAt || activity.createdAt, mode) === currentBucket)
    .forEach((activity) => {
      collectActivityContactIds(activity, dealMap).forEach((contactId) => {
...
export function createCampaignRecord(name, source = "hubspot") {
  ...
  return {
    ...
    prospecting: {
      ...
      ceciliaDisqualifiedNumbersCount: 0,
      dailyCallTarget: 15,
      ...
      ceciliaDisqualifiedNumberItems: [],
    },
    qualification: {
      totalLeads: 0,
      leadCount: 0,
...
export function getConfiguredCampaignSegmentListNames(value) {
  if (isPrimaryCampaignName(value)) {
    return PRIMARY_CAMPAIGN_SEGMENT_LIST_NAMES;
  }
...
function isSql(contact = {}) {
  ...
  if (
    lifecycle.includes("desqualificado")
    || lifecycle.includes("telefone incorreto")
    || leadStatus.includes("unqualified")
...
function isDisqualifiedContact(contact = {}) {
  const lifecycle = normalizeComparable(contact.lifecycleStage);
  return lifecycle.includes("desqualificado");
}

function isWrongNumberContact(contact = {}) {
  const lifecycle = normalizeComparable(contact.lifecycleStage);
  const leadStatus = normalizeComparable(contact.leadStatus);

  return lifecycle.includes("telefone incorreto")
    || leadStatus.includes("wrong number")
    || leadStatus.includes("numero invalido")
    || leadStatus.includes("invalid number");
}

function isUpdatedPhoneContact(contact = {}) {
  const lifecycle = normalizeComparable(contact.lifecycleStage);
  const leadStatus = normalizeComparable(contact.leadStatus);

  return lifecycle.includes("telefone atualizado")
    || leadStatus.includes("telefone atualizado")
    || leadStatus.includes("numero atualizado")
    || leadStatus.includes("phone updated")
    || leadStatus.includes("updated phone");
}

function isQualifiedAsNoByMercadoPrivado(contact = {}) {
  const qualifiedNoActors = Array.isArray(contact.qualifiedNoActors) ? contact.qualifiedNoActors : [];
  return qualifiedNoActors.some((actor) => normalizeComparable(actor) === normalizeComparable("Mercado Privado"));
}
...
function buildContactDetailItem(contact = {}, fallbackStatusLabel = "", fallbackDetailLabel = "Sem detalhe") {
  return {
    id: contact.id,
    ownerName: contact.ownerName || "Sem responsavel",
    leadName: contact.name || contact.email || "Lead sem nome",
    statusLabel: contact.leadStatus || contact.lifecycleStage || fallbackStatusLabel || "Sem status",
    detailLabel: contact.email || contact.phone || fallbackDetailLabel,
  };
}
...
function isCancelledMeeting(activity = {}) {
  const normalizedStatus = normalizeComparable(activity.status);
  const normalizedLabel = normalizeComparable(activity.statusLabel);

  return normalizedStatus === "canceled"
    || normalizedStatus === "cancelled"
    || normalizedStatus === "no_show"
    || normalizedStatus === "failed"
    || normalizedLabel === "cancelada"
    || normalizedLabel === "nao compareceu"
    || normalizedLabel === "falhou";
}

function isExplicitlyScheduledMeeting(activity = {}) {
  const normalizedStatus = normalizeComparable(activity.status);
  const normalizedLabel = normalizeComparable(activity.statusLabel);

  return normalizedStatus === "scheduled"
    || normalizedStatus === "rescheduled"
    || normalizedLabel === "agendada"
    || normalizedLabel === "programada"
    || normalizedLabel === "reagendada";
}

function isExplicitlyCompletedMeeting(activity = {}) {
  const normalizedStatus = normalizeComparable(activity.status);
  const normalizedLabel = normalizeComparable(activity.statusLabel);

  return normalizedStatus === "completed"
    || normalizedStatus === "busy"
    || normalizedLabel === "concluida"
    || normalizedLabel === "realizada";
}
...
function isScheduledMeeting(activity = {}) {
  ...
  if (isCancelledMeeting(activity) || isExplicitlyCompletedMeeting(activity)) {
    return false;
  }

  if (isExplicitlyScheduledMeeting(activity)) {
    return true;
  }

  const activityTimestamp = getActivityTimestamp(activity);
  return Number.isFinite(activityTimestamp) && activityTimestamp >= Date.now();
}
...
function isCompletedMeeting(activity = {}) {
  ...
  if (isCancelledMeeting(activity)) {
    return false;
  }

  if (isExplicitlyCompletedMeeting(activity)) {
    return true;
  }

  const activityTimestamp = getActivityTimestamp(activity);
  return Number.isFinite(activityTimestamp) && activityTimestamp < Date.now() && !isExplicitlyScheduledMeeting(activity);
}
...
export function buildCampaignSummaries({
  ...
  for (const campaign of campaignMap.values()) {
    ...
    const campaignDealMap = new Map(campaignDeals.map((deal) => [deal.id, deal]));

    const totalLeads = campaignContacts.length;
    const leadContacts = campaignContacts.filter((contact) => isLifecycleStage(contact, ["Lead"]));
    const prospectContacts = campaignContacts.filter((contact) => isLifecycleStage(contact, ["Prospect"]));
    ...
    const wrongNumberContacts = campaignContacts.filter(isWrongNumberContact);
    const updatedPhoneContacts = campaignContacts.filter(isUpdatedPhoneContact);
    const disqualifiedContacts = campaignContacts.filter(isDisqualifiedContact);
    const ceciliaDisqualifiedContacts = campaignContacts.filter(isQualifiedAsNoByMercadoPrivado);
    const contactIdsWithCalls = new Set(
      campaignActivities
        .filter((activity) => activity.kind === "call")
        .flatMap((activity) => collectActivityContactIds(activity, campaignDealMap)),
    );
    const uncontactedLeadContacts = campaignContacts.filter((contact) => !contactIdsWithCalls.has(contact.id));
    ...
    const sqlLeadItems = shouldIncludeCampaignDetail(includeDetails, "sqls", detailKey)
      ? campaignContacts
        .filter(isSql)
        .map((contact) => buildContactDetailItem(contact, "SQL", "Sem email"))
      : [];
    const leadItems = shouldIncludeCampaignDetail(includeDetails, "leadStage", detailKey)
      ? leadContacts.map((contact) => buildContactDetailItem(contact, "Lead", "Sem email"))
      : [];
    const prospectItems = shouldIncludeCampaignDetail(includeDetails, "prospects", detailKey)
      ? prospectContacts.map((contact) => buildContactDetailItem(contact, "Prospect", "Sem email"))
      : [];
    const uncontactedLeadItems = shouldIncludeCampaignDetail(includeDetails, "uncontactedLeads", detailKey)
      ? uncontactedLeadContacts.map((contact) => ({
        id: contact.id,
        ownerName: contact.ownerName || "Sem responsavel",
        leadName: contact.name || contact.email || "Lead sem nome",
        statusLabel: contact.lifecycleStage || contact.leadStatus || "Sem contato",
        detailLabel: contact.phone || contact.email || "Sem telefone",
      }))
      : [];
    ...
    const scheduledMeetingActivities = meetingActivities.filter(isScheduledMeeting);
    const completedMeetingActivities = meetingActivities.filter(isCompletedMeeting);
    ...
    const meetingItems = shouldIncludeCampaignDetail(includeDetails, "meetings", detailKey)
      ...
      : [];
    const completedMeetingItems = shouldIncludeCampaignDetail(includeDetails, "completedMeetings", detailKey)
      ? completedMeetingActivities
        .map((activity) => ({
          id: activity.id,
          ownerName: activity.ownerName || "Sem responsavel",
          leadName: activity.leadName || "Lead nao associado",
    ...
    const fourthAttemptItems = shouldIncludeCampaignDetail(includeDetails, "fourthAttemptContacts", detailKey)
      ...
      : [];
    const wrongNumberItems = shouldIncludeCampaignDetail(includeDetails, "wrongNumbers", detailKey)
      ? wrongNumberContacts.map((contact) => buildAttemptDetailItem(contact, contact.leadStatus || contact.lifecycleStage || "Numero errado"))
      : [];
    const updatedPhoneItems = shouldIncludeCampaignDetail(includeDetails, "updatedPhones", detailKey)
      ? updatedPhoneContacts.map((contact) => buildAttemptDetailItem(contact, contact.leadStatus || contact.lifecycleStage || "Telefone atualizado"))
      : [];
    const disqualifiedNumberItems = shouldIncludeCampaignDetail(includeDetails, "disqualifiedNumbers", detailKey)
      ...
      : [];
    const ceciliaDisqualifiedNumberItems = shouldIncludeCampaignDetail(includeDetails, "ceciliaDisqualifiedNumbers", detailKey)
      ? ceciliaDisqualifiedContacts.map((contact) => buildAttemptDetailItem(contact, "Qualificado? Nao"))
      : [];
    const meetingCount = scheduledMeetingActivities.length;
    const completedMeetingCount = completedMeetingActivities.length;
    const connectionActivities = campaignActivities.filter((activity) => activity.kind === "task");
    ...
    campaign.prospecting.callsDaily = countUniqueContactsWithActivity(
      campaignActivities,
      campaignContacts,
      campaignDealMap,
      "call",
    ...
    campaign.prospecting.connectionsDaily = countRecentActivities(campaignActivities, "call", "day");
    ...
    campaign.prospecting.wrongNumbersCount = wrongNumberContacts.length;
    campaign.prospecting.updatedPhoneCount = updatedPhoneContacts.length;
    campaign.prospecting.disqualifiedNumbersCount = disqualifiedContacts.length;
    campaign.prospecting.ceciliaDisqualifiedNumbersCount = ceciliaDisqualifiedContacts.length;
    ...
    campaign.prospecting.wrongNumberItems = wrongNumberItems;
    campaign.prospecting.updatedPhoneItems = updatedPhoneItems;
    campaign.prospecting.disqualifiedNumberItems = disqualifiedNumberItems;
    campaign.prospecting.ceciliaDisqualifiedNumberItems = ceciliaDisqualifiedNumberItems;
    campaign.qualification.totalLeads = totalLeads;
    campaign.qualification.totalLeadItems = totalLeadItems;
    campaign.qualification.leadCount = leadContacts.length;
    campaign.qualification.prospectCount = prospectContacts.length;
    campaign.qualification.leadItems = leadItems;
    campaign.qualification.prospectItems = prospectItems;
    ...
    campaign.qualification.uncontactedLeadCount = uncontactedLeadContacts.length;
    ...
    campaign.qualification.uncontactedLeadItems = uncontactedLeadItems;
    ...
    campaign.completedMeetings = completedMeetingItems;
    campaign.meetingCount = meetingCount;
    campaign.completedMeetingCount = completedMeetingCount;
...
export function aggregateCampaignSummary(campaigns = []) {
  ...
  campaigns.forEach((campaign) => {
    ...
    aggregate.prospecting.wrongNumbersCount += campaign.prospecting.wrongNumbersCount || 0;
    aggregate.prospecting.updatedPhoneCount += campaign.prospecting.updatedPhoneCount || 0;
    aggregate.prospecting.disqualifiedNumbersCount += campaign.prospecting.disqualifiedNumbersCount || 0;
    aggregate.prospecting.ceciliaDisqualifiedNumbersCount += campaign.prospecting.ceciliaDisqualifiedNumbersCount || 0;
    ...
    aggregate.prospecting.wrongNumberItems.push(...(campaign.prospecting.wrongNumberItems || []));
    aggregate.prospecting.updatedPhoneItems.push(...(campaign.prospecting.updatedPhoneItems || []));
    aggregate.prospecting.disqualifiedNumberItems.push(...(campaign.prospecting.disqualifiedNumberItems || []));
    aggregate.prospecting.ceciliaDisqualifiedNumberItems.push(...(campaign.prospecting.ceciliaDisqualifiedNumberItems || []));
    ...
    aggregate.qualification.leadCount += campaign.qualification.leadCount || 0;
    aggregate.qualification.prospectCount += campaign.qualification.prospectCount || 0;
    aggregate.qualification.leadItems.push(...(campaign.qualification.leadItems || []));
    aggregate.qualification.prospectItems.push(...(campaign.qualification.prospectItems || []));
    ...
    aggregate.qualification.uncontactedLeadCount += campaign.qualification.uncontactedLeadCount || 0;
    aggregate.qualification.sqlLeadItems.push(...(campaign.qualification.sqlLeadItems || []));
    aggregate.qualification.uncontactedLeadItems.push(...(campaign.qualification.uncontactedLeadItems || []));
    ...
    aggregate.completedMeetings.push(...(campaign.completedMeetings || []));
    aggregate.meetingCount += campaign.meetingCount;
    aggregate.completedMeetingCount += campaign.completedMeetingCount || 0;
