...
export function isUpcomingTask(task = {}, now = Date.now()) {
  const timestamp = getTaskTimestamp(task);
  return Number.isFinite(timestamp) && timestamp >= now && !task?.isCompleted;
}
...
export function getUpcomingTasks(tasks = [], now = Date.now()) {
...
export function getVisibleTasks(tasks = [], sessionUser = {}, {
  ...
  const scopedTasks = getScopedTasks(upcomingTasks, sessionUser);
