...
export function normalizeHubSpotContact(contact, ownerMap) {
  ...
  return {
    ...
    lifecycleStage: normalizeString(properties.lifecyclestage),
    leadStatus: normalizeString(properties.hs_lead_status),
  ...
  return owners
    .map((owner) => {
      ...
      const metaPercent = ownerDeals.length
        ...
        : 0;
      const hasOperationalPresence = ownerOpenDeals.length > 0 || pendingActivities.length > 0 || totalMeetings > 0;

      if (!hasOperationalPresence) {
        return null;
      }
...
export function buildDashboardDomainPayload(rawOwners, rawDeals, rawActivities = {}, rawPipelines = []) {
  ...
  const contacts = (rawActivities.contacts || []).map((contact) => normalizeHubSpotContact(contact, ownerMap));
